...
#if _MSC_VER >= 1700
bool CSerialException::GetErrorMessage(_Out_z_cap_(nMaxError)  char* lpszError, _In_ unsigned int nMaxError, _Out_opt_ unsigned int* pnHelpContext)
#else	
bool CSerialException::GetErrorMessage(__out_ecount_z(nMaxError) const char* lpszError, __in unsigned int nMaxError, __out_opt unsigned int* pnHelpContext)
#endif
{
  ...
  char* lpBuffer;
  unsigned long dwReturn = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
...
{
  CString rVal;
  const char* pstrError = rVal.GetBuffer(4096);
...
CSerialException::CSerialException(unsigned long dwError) : m_dwError(dwError)
{
}
...
void CSerialPort::ThrowSerialException(_In_ unsigned long dwError)
{
  if (dwError == 0)
...
#endif //#ifdef _DEBUG
#endif //#ifdef CSERIALPORT_MFC_EXTENSIONS
void AddLog2(const char* sz)
{
    //std::string fileName = commonfunction_c::BaseFunctions::getNowTimeString() + "_log.txt";
    //FILE *fp;
...
void CSerialPort::Open(_In_z_ const TCHAR* pszPort, _In_ unsigned long dwBaud, _In_ Parity parity, _In_ BYTE DataBits, _In_ StopBits stopBits, _In_ FlowControl fc, _In_ bool bOverlapped)
{
  Close(); //In case we are already open

  //Call CreateFile to open the comms port
  m_hComm = CreateFile(LPCWSTR(pszPort), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, bOverlapped ? FILE_FLAG_OVERLAPPED : 0, NULL);
  if (m_hComm == INVALID_HANDLE_VALUE)
  {
    ...
    {
        TCHAR Temp[200] = { 0 };
        _stprintf_s(Temp, 200, _T("%s ERROR_FILE_NOT_FOUND,Error Code:%d"), pszPort, GetLastError());
    ...
    {
        TCHAR Temp[200] = { 0 };
        _stprintf_s(Temp, 200, _T("%s ERROR_ACCESS_DENIED,Error Code:%d"), pszPort, GetLastError());
        MessageBox(NULL, Temp, _T("COM InitPort Error"), MB_ICONERROR);
        break;
    }
    ...
    {
        TCHAR Temp[200] = { 0 };
        _stprintf_s(Temp, 200, _T("%s ERROR_PATH_NOT_FOUND,Error Code:%d"), pszPort, GetLastError());
        MessageBox(NULL, Temp, _T("COM InitPort Error"), MB_ICONERROR);
        break;
    }
    default:
	...
	char ss[1024]={0};
	sprintf_s(ss,"CSerialPort::Open, Failed to open the comms port, Error:%u\n", dwLastError);
    AddLog2(ss);
  ...
  tm.ReadTotalTimeoutConstant = 1000;
  tm.ReadTotalTimeoutMultiplier = 500;
  ...
  AddLog2((char*)pszPort);
  AddLog2("\n");
  AddLog2("OpenCOM is OK\n");
  
}

void CSerialPort::Open(_In_ int nPort, _In_ unsigned long dwBaud, _In_ Parity parity, _In_ BYTE DataBits, _In_ StopBits stopBits, _In_ FlowControl fc, _In_ bool bOverlapped)
{
  //Form the string version of the port number
  TCHAR szPort[MAX_PATH];
  _stprintf_s(szPort, MAX_PATH, _T("\\\\.\\COM%d"), nPort);
...
unsigned long CSerialPort::Read(_Out_writes_bytes_(dwNumberOfBytesToRead) __out_data_source(FILE) void* lpBuffer, _In_ unsigned long dwNumberOfBytesToRead)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
  ...
  if(m_bOverlapped)
  {
	   ...
	   dwNumberOfBytesToRead = min(dwNumberOfBytesToRead,(unsigned long)ComStat.cbInQue);
	   if(!dwNumberOfBytesToRead)
		   return 0;  
		bool bReadStatus = ReadFile(m_hComm, lpBuffer, dwNumberOfBytesToRead, &dwBytesRead, &m_overlapped);
	  	
		if(!bReadStatus)
		{
		  if (GetLastError() == ERROR_IO_PENDING)
		  {
			GetOverlappedResult(m_overlapped, dwBytesRead, true);
			return dwBytesRead;
		  }
		}
  }
...
void CSerialPort::Read(_Out_writes_bytes_to_opt_(dwNumberOfBytesToRead, *lpNumberOfBytesRead) __out_data_source(FILE) void* lpBuffer, _In_ unsigned long dwNumberOfBytesToRead, _In_ OVERLAPPED& overlapped, _Inout_opt_ unsigned long* lpNumberOfBytesRead)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::ReadEx(_Out_writes_bytes_opt_(dwNumberOfBytesToRead) __out_data_source(FILE) LPVOID lpBuffer, _In_ unsigned long dwNumberOfBytesToRead, _Inout_ LPOVERLAPPED lpOverlapped, _In_ LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
VOID WINAPI CompletionRoutine(_In_ unsigned long dwErrorCode, _In_ unsigned long dwNumberOfBytesTransfered, _Inout_ LPOVERLAPPED lpOverlapped)
{
...
unsigned long CSerialPort::Write(_In_reads_bytes_opt_(dwNumberOfBytesToWrite) const void* lpBuffer, _In_ unsigned long dwNumberOfBytesToWrite)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
  ...
  {		  
	  ...
	  unsigned long dwErrors;
	  COMSTAT ComStat;
	  bool bWStat = WriteFile(m_hComm, lpBuffer, dwNumberOfBytesToWrite, &dwBytesWritten, &m_overlapped);

	  if(!bWStat)
	  {
		  if (GetLastError() == ERROR_IO_PENDING)
		  {
			  GetOverlappedResult(m_overlapped, dwBytesWritten, true);
...
void CSerialPort::Write(_In_reads_bytes_opt_(dwNumberOfBytesToWrite) const void* lpBuffer, _In_ unsigned long dwNumberOfBytesToWrite, _In_ OVERLAPPED& overlapped, _Out_opt_ unsigned long* lpNumberOfBytesWritten)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::WriteEx(_In_reads_bytes_opt_(dwNumberOfBytesToWrite) LPCVOID lpBuffer, _In_ unsigned long dwNumberOfBytesToWrite, _Inout_ LPOVERLAPPED lpOverlapped, _In_ LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::GetOverlappedResult(_In_ OVERLAPPED& overlapped, _Out_ unsigned long& dwBytesTransferred, _In_ bool bWait)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
unsigned long CSerialPort::BytesWaiting()
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::GetConfig(_In_ COMMCONFIG& config)
{
  ...
  unsigned long dwSize = sizeof(COMMCONFIG);
...
void CSerialPort::ClearError(_Out_ unsigned long& dwErrors)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::GetDefaultConfig(_In_ int nPort, _Out_ COMMCONFIG& config)
{
  ...
  char szPort[12];
  sprintf_s(szPort, sizeof(szPort)/sizeof(TCHAR), "COM%d", nPort);

  return GetDefaultConfig(szPort, config);
}

void CSerialPort::GetDefaultConfig(_In_z_ const char* pszPort, _Out_ COMMCONFIG& config)
{
  unsigned long dwSize = sizeof(COMMCONFIG);
  if (!GetDefaultCommConfig(LPCWSTR(pszPort), &config, &dwSize))
  {
    unsigned long dwLastError = GetLastError();
...
void CSerialPort::SetDefaultConfig(_In_z_ const char* pszPort, _In_ COMMCONFIG& config)
{
  unsigned long dwSize = sizeof(COMMCONFIG);
  if (!SetDefaultCommConfig(LPCWSTR(pszPort), &config, dwSize))
  {
    unsigned long dwLastError = GetLastError();
...
void CSerialPort::Escape(_In_ unsigned long dwFunc)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::GetModemStatus(_Out_ unsigned long& dwModemStatus)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::SetMask(_In_ unsigned long dwMask)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::GetMask(_Out_ unsigned long& dwMask)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::Purge(_In_ unsigned long dwFlags)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::Setup(_In_ unsigned long dwInQueue, _In_ unsigned long dwOutQueue)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
void CSerialPort::WaitEvent(_Inout_ unsigned long& dwMask)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
...
bool CSerialPort::WaitEvent(_Inout_ unsigned long& dwMask, _Inout_ OVERLAPPED& overlapped)
{
  //Validate our parameters
  //ATLASSERT(IsOpen());
